...
from alicemultiverse.events.base_v2 import BaseEvent
from alicemultiverse.events.asset_events_v2 import (
    AssetDiscoveredEvent, AssetProcessedEvent, AssetOrganizedEvent,
    QualityAssessedEvent, MetadataUpdatedEvent
)
from alicemultiverse.events.creative_events_v2 import (
    ProjectCreatedEvent, StyleChosenEvent, ContextUpdatedEvent,
    CharacterDefinedEvent, ConceptApprovedEvent
)
from alicemultiverse.events.workflow_events_v2 import (
    WorkflowStartedEvent, WorkflowStepCompletedEvent, 
...
class TestAllEventTypes:
    ...
    def test_event_serialization(self):
        ...
        events = [
            AssetDiscoveredEvent(file_path="/test.jpg", content_hash="abc", file_size=1024, media_type="image", project_name="test"),
            ProjectCreatedEvent(project_id="p1", project_name="Test", description="Desc"),
            WorkflowStartedEvent(workflow_id="w1", workflow_type="test", workflow_name="Test WF", initiated_by="user"),
            MetadataUpdatedEvent(content_hash="abc123", metadata={"test": "data"}),
            CharacterDefinedEvent(project_id="p1", character_id="c1", character_name="Alice", character_type="main"),
            WorkflowCompletedEvent(workflow_id="w1", workflow_type="test", total_duration_ms=10000, steps_completed=3)
        ]
    ...
    def test_event_type_names(self):
        ...
        for event_class, expected_type in event_type_map.items():
            ...
            if event_class == AssetDiscoveredEvent:
                event = event_class(file_path="/test", file_size=1, content_hash="abc", media_type="image", project_name="test")
            elif event_class == AssetProcessedEvent:
                event = event_class(content_hash="abc", file_path="/f")
            elif event_class == AssetOrganizedEvent:
                event = event_class(content_hash="abc", source_path="/s", destination_path="/d", project_name="p", source_type="st", date_folder="2024-01-01")
            elif event_class == QualityAssessedEvent:
                event = event_class(content_hash="abc", file_path="/f", star_rating=4, combined_score=0.75)
                ...
                event = event_class(project_id="p1", context_type="t", update_type="u", context_key="k")
            elif event_class == CharacterDefinedEvent:
                event = event_class(project_id="p1", character_id="c1", character_name="n", character_type="t")
            elif event_class == ConceptApprovedEvent:
                event = event_class(project_id="p1", concept_id="c1", concept_type="t", concept_name="n", description="d", approved_by="u")
                ...
                event = event_class(workflow_id="w1", step_id="s1", step_name="s", step_number=1, total_steps=3, status="ok")
            elif event_class == WorkflowCompletedEvent:
                event = event_class(workflow_id="w1", workflow_type="t", total_duration_ms=1000, steps_completed=3)
            elif event_class == WorkflowFailedEvent:
                event = event_class(workflow_id="w1", workflow_type="t", error_type="e", error_message="msg")
