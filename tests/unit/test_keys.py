...
from alicemultiverse.core.keys.cli import run_keys_command, run_interactive_setup
...
class TestAPIKeyManager:
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("key_name,env_var", [
...
class TestKeysCliCommands:
    ...
    def test_run_keys_set_command(self, mock_getpass, mock_input, mock_manager_class):
        ...
        assert result == 0
        mock_manager.set_api_key.assert_called_once_with("sightengine_user", "test-user", "keychain")
    ...
    def test_run_keys_list_command(self, mock_print, mock_manager_class):
        ...
        mock_manager = MagicMock()
        mock_manager.list_api_keys.return_value = [
            "claude (keychain)",
            "openai (config file)"
        ]
        mock_manager_class.return_value = mock_manager
