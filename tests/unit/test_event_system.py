...
from alicemultiverse.events import (
    Event, EventBus, EventSubscriber, publish_event, get_event_bus,
    AssetDiscoveredEvent, AssetProcessedEvent, QualityAssessedEvent,
    WorkflowStartedEvent, ProjectCreatedEvent, EventBuilder
)
from alicemultiverse.events.middleware import EventLogger, EventMetrics, EventFilter
...
class TestEventBus:
    ...
    async def test_wildcard_subscriber(self):
        ...
        await bus.publish(AssetDiscoveredEvent(
            file_path="/1.png", 
            content_hash="a",
            file_size=1000, 
            media_type="image", 
        ...
        await bus.publish(WorkflowStartedEvent(
            workflow_id="w1", 
            workflow_type="gen", 
            workflow_name="test"
        ).with_metadata(source="test"))
...
class TestEventMiddleware:
    ...
    async def test_event_filter(self):
        ...
        filter_middleware = EventFilter(
            include_types=['asset.discovered', 'asset.processed']
        )
