...
def create_sightengine_stage(api_user: str, api_secret: str, 
                            min_stars: int = 3,
                            weights: Optional[Dict[str, float]] = None,
                            star_thresholds: Optional[Dict[str, float]] = None) -> Callable:
        ...
        try:
            ...
            if result:
                ...
                if final_stars != previous_stars:
                    logger.info(f"{image_path.name}: Quality adjusted {previous_stars}★ → {final_stars}★")
...
def create_claude_stage(api_key: str, 
                       min_stars: int = 4,
    ...
    def process_claude(image_path: Path, metadata: Dict) -> Dict:
        ...
        try:
            ...
            if result:
                ...
                if final_stars != previous_stars:
                    logger.info(f"{image_path.name}: Final quality {previous_stars}★ → {final_stars}★")
...
def create_pipeline_stages(config: Any) -> List[Callable]:
    ...
    if pipeline_mode in ['standard', 'premium']:
        ...
        se_user = getattr(config.processing, 'sightengine_user', None) or key_manager.get_key('sightengine', 'user')
        se_secret = getattr(config.processing, 'sightengine_secret', None) or key_manager.get_key('sightengine', 'secret')
        
        if se_user and se_secret:
            stages.append(create_sightengine_stage(
                api_user=se_user,
    ...
    if pipeline_mode == 'premium':
        ...
        claude_key = getattr(config.processing, 'claude_api_key', None) or key_manager.get_key('claude')
        
        if claude_key:
            stages.append(create_claude_stage(
                api_key=claude_key,
                min_stars=4,
