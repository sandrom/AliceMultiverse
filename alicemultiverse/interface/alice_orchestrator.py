...
from ..events import (
    ...
    ProjectCreatedEvent, ContextUpdatedEvent, StyleChosenEvent,
    AssetDiscoveredEvent, WorkflowStartedEvent
)
from ..database import get_session
from ..database.repository import AssetRepository, ProjectRepository
from ..assets.hashing import calculate_content_hash
...
class AliceOrchestrator:
    ...
    def _parse_temporal_reference(self, text: str) -> Optional[datetime]:
    ...
    def _extract_creative_elements(self, text: str) -> Dict[str, Any]:
        ...
        # Style keywords
        style_keywords = [
            'cyberpunk', 'steampunk', 'fantasy', 'scifi', 'realistic',
            'anime', 'cartoon', 'abstract', 'surreal', 'minimalist',
            'retro', 'vintage', 'modern', 'futuristic', 'gothic'
        ]
        elements['styles'] = [s for s in style_keywords if s in text_lower]
        
        # Mood keywords
        mood_keywords = [
            'dark', 'bright', 'moody', 'cheerful', 'energetic',
            'calm', 'peaceful', 'dramatic', 'mysterious', 'playful',
            'serious', 'whimsical', 'melancholic', 'uplifting'
        ]
        ...
        color_keywords = [
            ...
            'pink', 'black', 'white', 'gray', 'neon', 'pastel',
            'vibrant', 'muted', 'monochrome', 'colorful'
        ]
    ...
    async def _handle_search(
        self, 
        request: str, 
        temporal_ref: Optional[datetime],
        ...
        try:
            ...
            if len(request) > 20 and not any(k in search_params for k in ['style_tags', 'mood_tags']):
    ...
    async def _handle_creation(
        ...
        try:
            ...
            # Update memory
            self._update_memory(CreativeIntent.CREATE, {
                'prompt': prompt,
                'style': creative_elements['styles'][0] if creative_elements['styles'] else 'default'
            })
            ...
            return CreativeResponse(
                ...
                suggestions=["Check if the generation service is available", "Try a simpler prompt"]
            )
    ...
    async def _handle_memory_request(self, request: str) -> CreativeResponse:
        ...
        try:
            ...
            # Creative patterns
                ...
                top_patterns = sorted(
                    self.memory.creative_patterns.items(),
                    key=lambda x: x[1],
                    reverse=True
                )[:5]
    ...
    async def save_context(self) -> bool:
        ...
        if self.project_id and self.project_repo:
            try:
                ...
                await publish_event(ContextUpdatedEvent(
                    source="AliceOrchestrator",
