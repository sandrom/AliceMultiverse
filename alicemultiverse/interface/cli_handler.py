...
def run_interface_command(args: Namespace) -> int:
    ...
    if args.demo:
        ...
        logger.info("")
        logger.info(
            "The Alice interface provides structured functions for AI assistants to interact with the media organization system."
        )
        logger.info("")
...
def run_demo() -> int:
    ...
    request = SearchRequest(style_tags=["cyberpunk"], subject_tags=["portrait"], limit=5)
    ...
    request = OrganizeRequest(
        source_path="~/Downloads", quality_assessment=True, enhanced_metadata=True
    )
    ...
    if response.get("data") and "processed_files" in response["data"]:
        ...
        asset_ids = [
            f["asset_id"] for f in response["data"]["processed_files"][:3] if "asset_id" in f
        ]
        if asset_ids:
            request = TagRequest(asset_ids=asset_ids, style_tags=["hero-shot"], role=AssetRole.HERO)
    ...
    request = SearchRequest(description="dark moody landscapes from last week")
    response = alice.search_assets(request)
