...
def handle_keys_command(args, config) -> int:
...
def handle_mcp_command(args, config) -> int:
...
def handle_migrate_command(args, config) -> int:
...
def handle_monitor_command(args, config) -> int:
...
def handle_storage_command(args, config) -> int:
...
def handle_scenes_command(args, config) -> int:
...
def handle_dedup_command(args, config) -> int:
    ...
    if args.dedup_command == "find":
        ...
        print(
            f"Total space that could be saved: {report['potential_savings_mb']:.1f} MB")
        ...
        stats = engine.remove_duplicates(
            report, strategy=args.strategy, backup_dir=Path(
                args.backup) if getattr(
                args, 'backup', None) else None, dry_run=getattr(
                    args, 'dry_run', True))
...
def handle_prompts_command(args, config) -> int:
...
def handle_organize_command(args, config) -> int:
...
def handle_index_command(args, config) -> int:
...
def handle_comparison_command(args, config) -> int:
...
def handle_transitions_command(args, config) -> int:
