...
Configuration overrides:
  %(prog)s --paths.inbox=~/Downloads --quality.thresholds.3_star.max=70
    ...
    keys_subparsers = keys_parser.add_subparsers(dest='keys_command', help='Key management commands')
    
    # Keys - set
    keys_set = keys_subparsers.add_parser('set', help='Set an API key')
    keys_set.add_argument('key_name', 
                         choices=['sightengine_user', 'sightengine_secret', 'anthropic_api_key'],
                         help='API key to set')
    keys_set.add_argument('--method',
    ...
    interface_parser = subparsers.add_parser('interface', help='Start Alice interface for AI interaction')
    interface_parser.add_argument(
        '--demo',
        action='store_true',
        help='Run demonstration of AI interaction'
    )
    ...
    mcp_parser.add_argument(
        '--port',
        type=int,
        help='Port to run on (for TCP mode, not implemented yet)'
    )
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "brisque-sightengine-claude", "full",
            # Custom
            "custom"
        ],
    ...
    parser.add_argument(
        "--stages",
        help="Custom pipeline stages (comma-separated)"
    )
    
    parser.add_argument(
        "--cost-limit",
        type=float,
        help="Maximum cost limit for API calls (in USD)"
    )
    
    parser.add_argument(
        "--sightengine-key",
        help="SightEngine API credentials (format: 'user,secret')"
    )
    ...
    try:
        ...
        if not config.paths.inbox:
            raise ConfigurationError(
                ...
                "Use --inbox or set paths.inbox in settings.yaml"
            )
        ...
        result = run_organizer(config, pipeline, stages, cost_limit, 
                             sightengine_key, claude_key)
