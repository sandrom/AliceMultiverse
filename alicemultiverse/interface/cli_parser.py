...
def add_understanding_args(parser) -> None:
    ...
    parser.add_argument(
        "--google-key",
        help="Google AI API key (overrides config)",
        dest="google_ai_api_key")
    parser.add_argument(
        "--deepseek-key",
...
def add_output_args(parser) -> None:
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug mode with development warnings")
    parser.add_argument(
...
def add_technical_args(parser) -> None:
    ...
    parser.add_argument(
        "--force-cli",
        action="store_true",
        help="Force CLI usage without deprecation warning")
...
def add_recreate_subcommand(subparsers) -> None:
    ...
    recreate_parser = subparsers.add_parser(
        "recreate", help="Recreate AI generations")
    recreate_subparsers = recreate_parser.add_subparsers(
        dest="recreate_command", help="Recreate commands"
    )

    # Recreate - inspect
    recreate_inspect = recreate_subparsers.add_parser(
        "inspect", help="Inspect generation metadata")
    recreate_inspect.add_argument("file", help="Image file to inspect")

    # Recreate - recreate
    recreate_recreate = recreate_subparsers.add_parser(
        "recreate", help="Recreate a generation")
    ...
    recreate_recreate.add_argument(
        "-v",
        "--variations",
        type=int,
        default=1,
    ...
    # Recreate - catalog
    recreate_catalog = recreate_subparsers.add_parser(
        "catalog", help="Catalog generations in directory")
...
def add_mcp_subcommand(subparsers) -> None:
    ...
    mcp_parser.add_argument(
        "--port",
        type=int,
        help="Port for network transport")
...
def add_migrate_subcommand(subparsers) -> None:
    ...
    migrate_legacy = migrate_subparsers.add_parser(
        "from-legacy", help="Migrate from legacy structure")
...
def add_monitor_subcommand(subparsers) -> None:
    ...
    monitor_parser = subparsers.add_parser(
        "monitor", help="Monitor AliceMultiverse operations")
    ...
    monitor_events = monitor_subparsers.add_parser(
        "events", help="Monitor real-time events")
    monitor_events.add_argument(
        "-f",
        "--follow",
        action="store_true",
        help="Follow events continuously")
...
def add_storage_subcommand(subparsers) -> None:
    ...
    storage_parser = subparsers.add_parser(
        "storage", help="Storage operations")
    storage_subparsers = storage_parser.add_subparsers(
        dest="storage_command", help="Storage commands"
    )

    # Storage - scan
    storage_scan = storage_subparsers.add_parser(
        "scan", help="Scan directories for assets")
    storage_scan.add_argument(
        "directories",
        nargs="+",
...
def add_scenes_subcommand(subparsers) -> None:
    ...
    # Scenes - shotlist
    scenes_shotlist = scenes_subparsers.add_parser(
        "shotlist", help="Generate shot list from scenes")
    scenes_shotlist.add_argument("scenes_file", help="Scenes JSON file")
    scenes_shotlist.add_argument(
        "-o",
        "--output",
        required=True,
        help="Output file")
...
def add_dedup_subcommand(subparsers) -> None:
    ...
    # Dedup - remove
    dedup_remove = dedup_subparsers.add_parser(
        "remove", help="Remove duplicate images")
    dedup_remove.add_argument("report", help="JSON report from find command")
    dedup_remove.add_argument(
        "-s",
        "--strategy",
        choices=[
            "safe",
...
def add_prompts_subcommand(subparsers) -> None:
    ...
    prompts_parser = subparsers.add_parser(
        "prompts", help="Manage AI prompts and their effectiveness")
    ...
    # Prompts - search
    prompts_search = prompts_subparsers.add_parser(
        "search", help="Search for prompts")
...
def add_index_subcommand(subparsers) -> None:
    ...
    index_rebuild = index_subparsers.add_parser(
        "rebuild", help="Rebuild search index from files")
    ...
    # Index - update
    index_update = index_subparsers.add_parser(
...
def add_comparison_subcommand(subparsers) -> None:
    ...
    comparison_parser = subparsers.add_parser(
        "comparison", help="Model comparison system")
    comparison_subparsers = comparison_parser.add_subparsers(
        dest="comparison_command", help="Comparison commands"
    )

    # Comparison - server
    comparison_server = comparison_subparsers.add_parser(
        "server", help="Start web interface")
    ...
    comparison_server.add_argument(
        "--populate-test-data",
        action="store_true",
        help="Populate with test data (development only)")

    # Comparison - populate
    comparison_populate = comparison_subparsers.add_parser(
        "populate", help="Populate with images")
    comparison_populate.add_argument("directory", help="Directory to scan")
    comparison_populate.add_argument(
        "-r",
        "--recursive",
        action="store_true",
        help="Search recursively")
    comparison_populate.add_argument(
        "-l", "--limit", type=int, help="Maximum number to add")

    # Comparison - populate-default
    comparison_populate_default = comparison_subparsers.add_parser(
        "populate-default", help="Populate from default dirs")
    comparison_populate_default.add_argument(
        "-l", "--limit", type=int, help="Maximum number to add")
...
def add_transitions_subcommand(subparsers) -> None:
    ...
    transitions_parser = subparsers.add_parser(
        "transitions", help="Analyze scene transitions")
    transitions_subparsers = transitions_parser.add_subparsers(
        dest="transitions_command", help="Transition analysis commands"
    )

    # Transitions - analyze
    transitions_analyze = transitions_subparsers.add_parser(
        "analyze", help="Analyze transitions between images")
    transitions_analyze.add_argument(
        "images",
        nargs="+",
        help="Image files to analyze (in sequence order)")
    transitions_analyze.add_argument(
    ...
    # Transitions - motion
    transitions_motion = transitions_subparsers.add_parser(
        "motion", help="Analyze motion in a single image")
    transitions_motion.add_argument("image", help="Image file to analyze")
    transitions_motion.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="Verbose output")
