...
from .constants import METADATA_DIR_NAME, CACHE_VERSION, HASH_CHUNK_SIZE
from .types import CacheMetadata, AnalysisResult
...
class MetadataCache:
    ...
    def load(self, media_path: Path) -> Optional[CacheMetadata]:
    ...
    def save(
        ...
        try:
            ...
            metadata: CacheMetadata = {
                ...
                'original_path': str(media_path) if not media_path.is_relative_to(self.source_root) else str(media_path.relative_to(self.source_root)),
    ...
    def set_metadata(self, media_path: Path, analysis: AnalysisResult, 
                     analysis_time: float = 0.0) -> None:
    ...
    def get_metadata(self, media_path: Path) -> Optional[CacheMetadata]:
    ...
    def _compute_quick_hash(self, file_path: Path) -> str:
        ...
        hash_str = f"{size}:{mtime}:{hashlib.md5(first_kb).hexdigest()}"
