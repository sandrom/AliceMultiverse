...
class ProviderPricing:
    ...
    # Pricing models
    per_request: float | None = None          # Fixed cost per request
    per_token_input: float | None = None      # Cost per input token
    per_token_output: float | None = None     # Cost per output token
    per_second: float | None = None           # Cost per second (video/audio)
    per_megapixel: float | None = None        # Cost per megapixel (images)
...
class CostTracker:
    ...
    def get_provider_comparison(self, category: CostCategory | None = None) -> list[dict[str, Any]]:
