...
class MetadataCacheAdapter:
    ...
    def get_metadata(self, media_path: Path) -> Optional[Dict[str, Any]]:
...
class EnhancedMetadataCacheAdapter(MetadataCacheAdapter):
    ...
    def load_enhanced(self, media_path: Path) -> Optional[Any]:
        """Load enhanced metadata."""
        return self._unified.load_enhanced(media_path)
    
    def save_enhanced(self, media_path: Path, analysis: Any, analysis_time: float, enhanced_metadata: Optional[Any] = None) -> None:
    ...
    def search_by_tags(self, tags: list, tag_type: Optional[str] = None) -> list:
        """Search by tags."""
...
class PersistentMetadataManagerAdapter:
    ...
    def __init__(self, cache_dir: Path, quality_thresholds: Optional[Dict[str, float]] = None, project_id: str = 'default'):
        """Initialize adapter."""
        self._unified = UnifiedCache(
        ...
        self.cache = EnhancedMetadataCacheAdapter(
            source_root=cache_dir,
            project_id=project_id
        )
