...
from ..monitoring.tracker import track_cache_access, track_operation
...
class UnifiedCache:
    ...
    def load(self, media_path: Path) -> dict[str, Any] | None:
        ...
        with track_operation("cache.load_file"):
            result = self.load_from_cache(media_path)
    ...
    def get_metadata(self, media_path: Path) -> dict[str, Any] | None:
        """Get metadata for a file (alias for load)."""
        return self.load(media_path)
    
    def set_metadata(self, media_path: Path, metadata: dict[str, Any], analysis_time: float = 0.0) -> None:
        """Set metadata for a file (alias for save)."""
        self.save(media_path, metadata, analysis_time)
    
    def update_stats(self, cache_hit: bool, analysis_time: float = 0.0) -> None:
        """Update cache statistics.
        
        Args:
            cache_hit: Whether this was a cache hit
            analysis_time: Time saved (for hits) or spent (for misses)
        """
        if cache_hit:
            self.cache_hits += 1
    ...
    def save(self, media_path: Path, analysis: AnalysisResult, analysis_time: float) -> None:
        ...
        with track_operation("cache.save"):
            ...
            # Need to actually save to cache file
    ...
    def load_enhanced(self, media_path: Path) -> AssetMetadata | None:
        ...
        if basic is not None and "enhanced_metadata" in basic:
    ...
    def rebuild_from_images(self, image_dir: Path) -> int:
        ...
        for img_path in image_dir.rglob("*"):
            ...
            if metadata and "alice_metadata" in metadata:
                ...
                self.save(img_path, analysis, analysis_time)
                count += 1
    ...
    def load_from_cache(self, media_path: Path) -> dict[str, Any] | None:
        ...
        Returns:
            Cached metadata if available, None otherwise
        """
        content_hash = get_file_hash(media_path)
        cache_file = self.cache_dir / f"{content_hash}.json"
        ...
        if cache_file.exists():
            try:
                with open(cache_file) as f:
                    ...
                    return metadata.get("analysis", metadata)
            except Exception as e:
