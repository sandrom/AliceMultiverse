...
from ..core.types import AnalysisResult, OrganizeResult, MediaType
...
class EnhancedMediaOrganizer(MediaOrganizer):
    ...
    def __init__(self, config: DictConfig):
        ...
        if self.embed_metadata:
            cache_dir = Path(config.paths.get('cache_dir', Path.home() / '.alicemultiverse' / 'cache'))
        ...
        if result['status'] == 'success':
            try:
                ...
                enhanced_metadata = self.metadata_cache.extractor.extract_metadata(
                    media_path,
                    analysis,
                    self.metadata_cache.project_id,
                    content_hash
                )
                ...
                if self.persistent_metadata and result.get('destination'):
                    ...
                    if output_path.exists():
                        ...
                        embed_metadata = {
                            ...
                            'role': enhanced_metadata.get('role').value if hasattr(enhanced_metadata.get('role'), 'value') else enhanced_metadata.get('role'),
                        ...
                        if 'sightengine_quality' in result:
                            embed_metadata.update({
                                ...
                                'sightengine_ai_generated': result.get('sightengine_ai_generated'),
                                'sightengine_ai_probability': result.get('sightengine_ai_probability'),
                            })
                        ...
                        # Save to image
                        success = self.persistent_metadata.save_metadata(output_path, embed_metadata)
                        if success:
    ...
    def _detect_relationships(self, metadata: AssetMetadata) -> None:
        ...
        if any(pattern in filename for pattern in ['_v2', '_v3', 'variation', 'alt']):
            ...
            base_name = filename.split('_v')[0] if '_v' in filename else filename.replace('variation', '').replace('alt', '')
    ...
    def group_assets(self, asset_ids: List[str], group_name: str) -> bool:
        ...
        for i, asset_id in enumerate(asset_ids):
            ...
            if not self.metadata_cache.update_metadata(
                asset_id,
                {'grouped_with': other_assets, 'custom_tags': [f"group:{group_name}"]}
            ):
                success = False
    ...
    def get_organization_summary(self) -> str:
        ...
        if self.search_engine:
            ...
            with_relationships = sum(1 for m in all_metadata.values() 
                                   if m.get('parent_id') or m.get('similar_to'))
