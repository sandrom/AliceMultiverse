...
from ..core.types import MediaType, AnalysisResult, OrganizeResult, Statistics
...
from ..quality.brisque import BRISQUEAssessor, is_available as brisque_available
...
from .organization_helpers import (
    get_quality_folder_name, extract_project_folder,
    create_organize_result, match_ai_source_patterns,
    get_possible_quality_locations
)
from ..events import (
...
class MediaOrganizer:
    ...
    def _process_file(self, media_path: Path) -> OrganizeResult:
        ...
        try:
            ...
            if dest_path.exists():
                return self._handle_duplicate_destination(media_path, dest_path, analysis, project_folder)
            
            # Handle existing file in different location
            if existing_file and existing_file != dest_path:
                return self._handle_existing_file_relocation(media_path, existing_file, dest_path, analysis, project_folder)
    ...
    def _find_existing_organized_file(
        ...
        if base_dir.exists():
            ...
            for stars in range(1, 6):
                ...
                if star_dir.exists():
                    possible_locations.extend(star_dir.glob(f"{project_folder}-*{source_path.suffix}"))
            
            # Check each possible location
            for existing_path in possible_locations:
                if existing_path.is_file() and self.file_handler.files_are_identical(source_path, existing_path):
        ...
        for date_dir in self.output_dir.iterdir():
            ...
            if project_dir.exists():
                ...
                # Direct location
                possible_locations.extend(project_dir.glob(f"{project_folder}-*{source_path.suffix}"))
    ...
    def _cleanup_duplicates(self) -> None:
        ...
        if duplicates_removed > 0:
            logger.info(f"Removed {duplicates_removed} duplicate files, saved {space_saved / 1024 / 1024:.1f} MB")
        ...
        for stage in self.pipeline_stages:
            try:
                ...
                if current_stars >= min_stars:
                    ...
                    if self.pipeline_cost_limit and self.pipeline_cost_total + stage_cost > self.pipeline_cost_limit:
                ...
                logger.error(f"Error in pipeline stage {getattr(stage, 'stage_name', 'unknown')}: {e}")
    ...
    def _log_statistics(self) -> None:
        ...
        if self.stats['moved_existing'] > 0:
            ...
            logger.info(f"    - Newly organized: {self.stats['organized'] - self.stats['moved_existing']}")
    ...
    def _get_or_analyze_media(self, media_path: Path, project_folder: str) -> dict:
        ...
        if cached_metadata:
            ...
            if 'file_number' not in analysis or analysis['file_number'] is None:
                analysis['file_number'] = self._get_next_file_number(project_folder, analysis['source_type'])
                # Update cache with the new file number
                self.metadata_cache.set_metadata(media_path, analysis, cached_metadata.get('analysis_time', 0))
    ...
    def _handle_duplicate_destination(self, media_path: Path, dest_path: Path, 
                                    analysis: dict, project_folder: str) -> OrganizeResult:
    ...
    def _handle_existing_file_relocation(self, media_path: Path, existing_file: Path,
                                       dest_path: Path, analysis: dict, 
                                       project_folder: str) -> OrganizeResult:
        ...
        if not self.config.processing.dry_run:
            ...
            if dest_path.exists() and self.file_handler.files_are_identical(existing_file, dest_path):
    ...
    def _perform_file_operation(self, media_path: Path, dest_path: Path,
                               analysis: dict, project_folder: str) -> OrganizeResult:
