...
class SightEngineStage(PipelineStage):
    ...
    def __init__(self, api_user: str, api_secret: str, min_stars: int = 3,
                 weights: Optional[Dict[str, float]] = None,
                 star_thresholds: Optional[Dict[str, float]] = None):
    ...
    def process(self, image_path: Path, metadata: Dict) -> Dict:
        ...
        try:
            ...
            if result:
                ...
                combined_score = (brisque_weight * brisque_normalized) + (sightengine_weight * sightengine_score)
                ...
                if final_stars != brisque_stars:
                    logger.info(f"{image_path.name}: Quality adjusted from {brisque_stars}★ to {final_stars}★ "
...
class ClaudeStage(PipelineStage):
    ...
    def __init__(self, api_key: str, min_stars: int = 4, model: str = "claude-3-haiku-20240307",
                 weights: Optional[Dict[str, float]] = None,
                 star_thresholds: Optional[Dict[str, float]] = None):
        ...
        try:
            ...
            if result:
                ...
                if final_stars != previous_stars:
                    logger.info(f"{image_path.name}: Final quality: {final_stars}★ "
