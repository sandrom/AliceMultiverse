...
from ..core.types import OrganizeResult, MediaType
...
class PipelineOrganizer(MediaOrganizer):
    ...
    def _create_stage(self, stage_name: str) -> Optional[PipelineStage]:
        ...
        if stage_name == 'brisque':
            ...
            if not api_user or not api_secret:
                logger.error("SightEngine API credentials not found. Use 'alice keys setup' to configure.")
                return None
            ...
            return ClaudeStage(api_key, min_stars, "claude-3-haiku-20240307", weights, star_thresholds)
    ...
    def _build_destination_path(
        ...
        dest_path = super()._build_destination_path(
            source_path, date_taken, project_folder, 
            source_type, file_number, quality_stars
        )
    ...
    def _process_file(self, media_path: Path) -> OrganizeResult:
        ...
        try:
            ...
            if cached_metadata and not self.config.processing.get('force_reindex', False):
                ...
                if analysis.get('media_type') == MediaType.IMAGE and analysis.get('quality_stars') is not None:
                    # Process through pipeline stages
                    metadata = dict(analysis)
                    updated = False
                    ...
                    for stage in self.pipeline_stages:
                        ...
                        if stage.name() in existing_stages:
                            logger.debug(f"Skipping {stage.name()} for {media_path.name} - already processed")
                            continue
