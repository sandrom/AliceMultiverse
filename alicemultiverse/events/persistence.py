...
class EventPersistence:
    ...
    async def persist_event(self, event: BaseEvent) -> str:
        ...
        entry_id = await self._redis.xadd(
            stream_key, {"event": event_json}, maxlen=self.max_len, approximate=True
        )

        # Also store in global stream for monitoring
        await self._redis.xadd(
            self._get_global_stream_key(),
            {"event": event_json, "type": event.event_type},
    ...
    async def get_events(
        ...
        for entry_id, data in entries:
            ...
            events.append({"stream_id": entry_id, "event": event_data})

        return events

    async def consume_events(
        self, event_types: list[str], consumer_name: str, block_ms: int = 1000, count: int = 10
        ...
        while True:
            try:
                ...
                messages = await self._redis.xreadgroup(
                    self.consumer_group, consumer_name, streams, count=count, block=block_ms
                )
    ...
    async def get_pending_events(
        self, event_type: str, consumer_name: str | None = None, idle_ms: int = 60000
    ) -> list[dict[str, Any]]:
        ...
        pending = await self._redis.xpending_range(
            stream_key, self.consumer_group, min="-", max="+", count=100, consumername=consumer_name
        )
    ...
    async def trim_old_events(self, event_type: str, max_age_days: int = 30):
        ...
        # Trim stream
        await self._redis.xtrim(stream_key, minid=f"{cutoff_ms}-0", approximate=False)
...
_persistence: EventPersistence | None = None


async def get_persistence(redis_url: str | None = None) -> EventPersistence:
