...
class LightingAnalysis:
    ...
    time_of_day: Optional[str] = None  # golden_hour, blue_hour, midday, night
...
class StyleAnalyzer:
    ...
    def _find_focal_points(self, gray: np.ndarray) -> List[Tuple[float, float]]:
        ...
        if len(corner_coords[0]) > 0:
            points = list(zip(corner_coords[1] / w, corner_coords[0] / h))
    ...
    def _guess_time_of_day(self, image: np.ndarray, l_channel: np.ndarray) -> Optional[str]:
