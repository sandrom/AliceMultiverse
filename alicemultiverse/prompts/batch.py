...
class PromptBatchProcessor:
    ...
    def batch_analyze_effectiveness(self,
        ...
        for prov, data in by_provider.items():
            data["success_rate"] = data["total_successes"] / data["total_uses"] if data is not None and data["total_uses"] > 0 else 0
            data["avg_rating"] = sum(data["ratings"]) / len(data["ratings"]) if data is not None and data["ratings"] else None
