...
from .hashing import get_or_calculate_content_hash, calculate_content_hash
...
class AssetDiscovery:
    ...
    def find_asset(self, content_hash: str) -> Optional[Path]:
    ...
    def register_asset(self, file_path: Path, project_id: Optional[str] = None) -> str:
    ...
    def scan_directory(self, directory: Path, project_id: Optional[str] = None) -> List[str]:
    ...
    def _scan_for_asset(self, content_hash: str) -> Optional[Path]:
    ...
    def _record_asset_path(self, session: Session, content_hash: str, file_path: Path) -> None:
        ...
        existing = session.query(AssetPath).filter_by(
            content_hash=content_hash,
            file_path=str(file_path)
        ).first()
