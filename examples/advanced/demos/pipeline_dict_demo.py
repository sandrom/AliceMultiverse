...
metadata = {"file_name": "test.png", "quality_stars": 4}
...
for i in range(3):
    old_metadata["pipeline_stages"].append(
        {"name": "brisque", "score": 35.0, "stars": 4, "passed": True}
    )

print(
    f"Number of BRISQUE entries: {len([s for s in old_metadata['pipeline_stages'] if s['name'] == 'brisque'])}"
)
...
print(
    f"Number of BRISQUE entries: {len([k for k in new_metadata['pipeline_stages'] if k == 'brisque'])}"
)
