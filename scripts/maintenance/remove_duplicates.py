...
def remove_duplicates(duplicates: dict[str, list[Path]], dry_run: bool = True) -> None:
    ...
    logger.info(
        f"Space {'would be' if dry_run else ''} saved: {total_size_saved:,} bytes ({total_size_saved / 1024 / 1024:.1f} MB)"
    )
...
def main():
    ...
    parser.add_argument(
        "--remove", action="store_true", help="Actually remove duplicates (default is dry run)"
    )
