...
SENSITIVE_PATTERNS: List[Tuple[str, Pattern]] = [
    # API Keys and Tokens
    ("API Key", re.compile(r'(?i)(api[_\s-]?key|apikey|api[_\s-]?token|access[_\s-]?token)\s*[:=]\s*["\']?[a-zA-Z0-9_\-]{20,}["\']?', re.IGNORECASE)),
    ("AWS Key", re.compile(r'(?i)(aws[_\s-]?access[_\s-]?key[_\s-]?id|aws[_\s-]?secret[_\s-]?access[_\s-]?key)\s*[:=]\s*["\']?[A-Z0-9]{20,}["\']?')),
    ("Private Key", re.compile(r'-----BEGIN\s+(RSA|DSA|EC|OPENSSH|PGP)\s+PRIVATE\s+KEY-----')),
    
    # Passwords
    ("Password", re.compile(r'(?i)(password|passwd|pwd)\s*[:=]\s*["\']?[^"\']{8,}["\']?')),
    ("Database URL", re.compile(r'(?i)(mysql|postgresql|postgres|mongodb|redis)://[^:]+:[^@]+@[^/]+')),
    
    # Personal Information
    ("Email", re.compile(r'\b[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(com|org|net|edu|gov|mil|int|co\.[a-z]{2})\b')),
