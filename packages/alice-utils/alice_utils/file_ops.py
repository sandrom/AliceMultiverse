...
def hash_file_content(file_path: str | Path) -> str:
    """Compute SHA256 hash of file content."""
    return compute_file_hash(file_path)


def copy_with_metadata(src: str | Path, dst: str | Path) -> None:
...
def atomic_write(file_path: str | Path, content: str | bytes, mode: str = "w") -> None:
    """Write file atomically using a temporary file."""
    file_path = Path(file_path)
    file_path.parent.mkdir(parents=True, exist_ok=True)

    # Write to temporary file first
    with tempfile.NamedTemporaryFile(mode=mode, dir=file_path.parent, delete=False) as tmp_file:
