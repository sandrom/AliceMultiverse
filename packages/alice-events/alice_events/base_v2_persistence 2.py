...
from .base_v2 import EventBus, BaseEvent, EventSubscriber
...
class PersistentEventBus(EventBus):
    ...
    async def replay_events(self, event_type: str, 
                          start: str = "-",
                          end: str = "+",
        ...
        events = await self.persistence.get_events(
            event_type, start=start, end=end, count=count
        )
    ...
    async def start_consumer(self, event_types: List[str],
                           consumer_name: str,
...
async def monitor_events_from_redis(event_types: List[str] = None):
    ...
    # Start consuming
    await persistent_event_bus.start_consumer(
        event_types, 
        "event-monitor",
        monitor
    )
