...
from alice_utils import compute_file_hash, detect_media_type, extract_image_metadata, get_file_info
...
class AssetAnalyzer:
    ...
    async def analyze(self, file_path: Path) -> AnalyzeResponse:
        ...
        if media_type == MediaType.IMAGE:
            ...
            if "width" in img_metadata and "height" in img_metadata:
                dimensions = {"width": img_metadata["width"], "height": img_metadata["height"]}
    ...
    def _detect_ai_source_from_metadata(self, info: dict[str, Any]) -> SourceType | None:
    ...
    def _detect_ai_source_from_filename(self, filename: str) -> SourceType | None:
        """Detect AI source from filename patterns."""
        filename_lower = filename.lower()

        # Common filename patterns
        filename_patterns = {
            SourceType.STABLE_DIFFUSION: [r"sd_", r"stable_diffusion", r"_sd\d+", r"automatic1111"],
            SourceType.MIDJOURNEY: [r"mj_", r"midjourney", r"_mj_", r"discord"],
            SourceType.DALLE: [r"dalle", r"dall-e", r"openai"],
            SourceType.COMFYUI: [r"comfyui", r"comfy_"],
            SourceType.LEONARDO: [r"leonardo", r"leo_ai"],
            SourceType.FLUX: [r"flux_", r"_flux"],
        }
    ...
    def _extract_semantic_tags(
        self, metadata: dict[str, Any], gen_params: dict[str, Any]
    ) -> list[str]:
