...
class AnalyzeResponse(BaseModel):
    ...
    # Detected AI source
    ai_source: SourceType | None = Field(description="Detected AI generation source")
    ...
    extracted_metadata: dict[str, Any] = Field(
        default_factory=dict, description="Extracted technical metadata (EXIF, etc.)"
    )

    # AI generation parameters
    generation_params: dict[str, Any] = Field(
        default_factory=dict, description="AI generation parameters if found"
    )
    ...
    dimensions: dict[str, int] | None = Field(
        default=None, description="Width and height for images/video"
    )
...
class QualityAssessRequest(BaseModel):
    ...
    pipeline_mode: str = Field(
        default="basic", description="Pipeline mode: basic, standard, premium"
    )
    force_reanalyze: bool = Field(default=False, description="Force re-analysis even if cached")
...
class QualityAssessResponse(BaseModel):
    ...
    # Individual scores
    brisque_score: float | None = Field(
        default=None, description="BRISQUE score (0-100, lower is better)"
    )
    sightengine_score: float | None = Field(
        default=None, description="SightEngine score (0-1, higher is better)"
    )
    claude_assessment: dict[str, Any] | None = Field(
        default=None, description="Claude's quality assessment"
    )

    # Quality issues
    quality_issues: list[str] = Field(default_factory=list, description="Detected quality issues")

    # Pipeline info
    pipeline_mode: str = Field(description="Pipeline mode used")
    stages_completed: list[str] = Field(
        default_factory=list, description="Completed pipeline stages"
    )
...
class OrganizePlanRequest(BaseModel):
    ...
    quality_rating: QualityRating | None = Field(
        default=None, description="Quality rating if assessed"
    )
    project_name: str | None = Field(default=None, description="Project name override")
...
class OrganizePlanResponse(BaseModel):
    ...
    quality_folder: str | None = Field(default=None, description="Quality folder (e.g., '5-star')")
    ...
    organization_rule: str = Field(
        default="date-project-source", description="Organization rule applied"
    )
    preserve_original_name: bool = Field(
        default=False, description="Whether to preserve original filename"
    )
